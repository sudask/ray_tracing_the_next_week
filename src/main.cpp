    ...
    for(int a = -10; a < 10; a++) {
        for(int b = -10; b < 10; b++) {
            ...
            if((center - vec3(4, 0.2, 0)).length() > 0.9) {
                if(choose_mat < 0.8) {
                    list[i++] = new moving_sphere(center, center + vec3(0, 0.5 * get_random_float(), 0), 0.0, 1.0, 0.2, new lambertian(new constant_texture(vec3(get_random_float() * get_random_float(), get_random_float() * get_random_float(), get_random_float() * get_random_float()))));
                }
